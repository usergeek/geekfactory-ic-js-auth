...
const Helper = {
    ...
    getLoggedInPrincipal: async (whitelist = undefined, host) => {
        try {
            ...
            if (wallet) {
                ...
                if (!connected || !wallet.agent) {
                    await wallet.requestConnect({
                        host: host !== null && host !== void 0 ? host : defaultHost,
                        whitelist: whitelist,
                    });
                }
    ...
    login: async (whitelist = undefined, host) => {
        try {
            const wallet = getGlobalWallet();
            if (wallet) {
                await wallet.requestConnect({
    ...
    createActor: async (canisterId, interfaceFactory, host) => {
        const parameters = {
