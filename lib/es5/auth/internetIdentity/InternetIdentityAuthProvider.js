...
const InternetIdentityAuthProvider = (props) => {
    ...
    const login = (0, react_1.useCallback)(async (identityProviderURL) => {
        try {
            (0, react_dom_1.unstable_batchedUpdates)(() => {
                authSourceProviderContext.setSource(props.source);
                updateContextStatus({ inProgress: true });
            });
            const authClient = await AuthClientFacade_1.AuthClientFacade.provideAuthClient();
            if (authClient) {
                const identity = await AuthClientFacade_1.AuthClientFacade.login(authClient, identityProviderURL, props.source === "II" ? "II" : "NFID");
                if (identity) {
    ...
    return React.createElement(props.context.Provider, { value: value }, props.children);
};
exports.InternetIdentityAuthProvider = InternetIdentityAuthProvider;
const getIdentityAccounts = async (identity, source) => {
    try {
        return [{
